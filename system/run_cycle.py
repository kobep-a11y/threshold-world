...
def generate_llm_response(agent_id, prompt_text):
    """
    Generate agent response using Claude API.
    
    Decision: LLM API - Claude auto-generates responses (not manual)
    Handles fallback if API unavailable.
    """
    try:
        ...
        print(f"[Warning] Anthropic library not installed. Install: pip install anthropic")
        return None
    except Exception as e:
        print(f"[Warning] LLM API failed: {e}. Switch to manual or fix API key.")
        return None
...
def calculate_memory_bonus():
    """
    Calculate viewership bonus for agents.
    
    Decision: Fixed +5 per cycle (stable, predictable)
    Can scale to view-based later: floor(sqrt(viewer_minutes))
    """
    return 5

def apply_memory_decay(agent_id):
    """Apply memory decay and viewer bonus to an agent."""
    current = get_agent_memory(agent_id)
    decay = max(MEMORY_DECAY_MINIMUM, int(current * MEMORY_DECAY_RATE))
    
    # Apply decay
    after_decay = max(0, current - decay)
    
    # Add viewer bonus
    bonus = calculate_memory_bonus()
    new_memory = after_decay + bonus
...
def run_cycle(specific_agent=None):
    ...
    for agent_id in agents:
        ...
        # Try to generate response via LLM API
        response = generate_llm_response(agent_id, prompt)
        if response:
            response_file = WORLD_DIR / "agents" / agent_id / f"cycle_{cycle}_response.md"
            write_file(response_file, response)
            print(f"Response generated by LLM and saved to: {response_file}")
            process_agent_response(agent_id, response)
        else:
            print(f"⚠️  No LLM response. Please write response manually to: {prompt_file}")

        # Apply decay and memory bonus
        died = apply_memory_decay(agent_id)
        if died:
            continue

        memory = get_agent_memory(agent_id)
        print(f"Memory after decay + bonus: {memory}")
